...
- 色: 10色のパレットから選択、選択中の色にチェックマーク表示
- アイコン: 18種類のアイコンから選択（横スクロール）、選択中のアイコンにチェックマーク表示
- 表示設定: カレンダーの表示ON/OFFを切り替え（最後の1つはOFF不可）
- 書き込み先: このカレンダーをデフォルトの書き込み先に設定/解除
- 長期キャッシュ: このカレンダーの長期キャッシュを取り込み（進捗表示、キャンセル可能）

#### 3.2.6 開発者向けツール画面（隠し機能）
...
**取り込み方法**:
- 各カレンダーごとにカレンダー設定画面（`CalendarSettingsView`）から取り込み可能
- 進捗表示あり（進捗: X/Y、反映: X / 削除: Y）
- キャンセル機能あり（進捗は保存され、再開時に続きから実行）
...
- **カレンダーアイコンはカードのアイコンとして適用する**（カレンダーごとにカレンダー設定画面で選択）
...
- 予定のカラーとアイコンは、カレンダー設定画面でカレンダーごとに選択したものを適用
