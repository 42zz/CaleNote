...
### 追加機能
- **設定画面の機能を拡充**（CAL-11）
  - **同期範囲設定**: 過去・未来の同期日数をUIから設定可能に（1〜365日、7日刻み）
  - **表示設定**: 週の開始曜日を日曜日/月曜日から選択可能に
  - **データ管理**: ローカルデータ再構築とキャッシュクリア機能を追加
  - **アプリ情報**: プライバシーポリシーと利用規約へのリンクを追加

### 実装詳細
- `DisplaySettings.swift`: 週の開始曜日設定の保存・読み込み機能を新規作成
- `CalendarSyncState.clearAllTokens`: 複数カレンダーの同期トークン一括削除機能を追加
- `SettingsView`:
  - 「同期範囲」セクション: Stepperで過去/未来日数を設定、SyncSettingsに即時保存
  - 「表示設定」セクション: Pickerで週の開始曜日を選択、DisplaySettingsに即時保存
  - 「データ管理」セクション: ローカルデータ再構築（同期トークンリセット）とキャッシュクリア（CachedCalendarEvent/ArchivedCalendarEvent全削除）
  - 「アプリ情報」セクション: プライバシーポリシー・利用規約への外部リンク

### 受け入れ条件達成
- ✅ アカウント管理セクションがある（既存）
- ✅ 同期設定が実装されている（同期範囲設定を追加）
- ✅ 同期失敗エントリーの再送機能がある（既存）
- ✅ ローカルデータ再構築機能がある（新規追加）
- ✅ アプリ情報が表示される（新規追加）
