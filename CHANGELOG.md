...
### 追加機能
- **データ復旧機能を実装（CAL-15）**
  - ローカルデータ破損時にGoogle Calendarから完全再構築
  - 設定画面から手動で復旧を実行可能
  - 復旧プロセスの進捗表示
  - ジャーナルは保持したままキャッシュのみを再構築

- **データベース整合性チェック機能を実装**
  - 起動時に自動でデータ整合性をチェック
  - 孤立したジャーナル（リンク先イベントが存在しない）を検出
  - カレンダー情報やイベントデータの欠損を検知
  - 問題検出時に設定画面への誘導アラートを表示

### 実装詳細
- `DataRecoveryService`: データ復旧のオーケストレーション
  - `checkIntegrity()`: 整合性チェック（JournalEntry/CachedCalendarEvent/ArchivedCalendarEvent/CachedCalendarのカウントと孤立検出）
  - `performFullRecovery()`: 完全再構築（ローカルデータ削除→カレンダー一覧同期→イベント取得→インデックス再構築）
  - 復旧ログをSyncLog（syncType="recovery"）に記録
  - キャンセルサポート（Task.checkCancellation）
  - エラー時はCrashlyticsに送信

- `SettingsView`: 「データ復旧」セクションを追加
  - 「Google Calendarから再構築」ボタン
  - 確認ダイアログで破壊的操作を防止
  - 復旧中の進捗表示（フェーズ、カレンダー、件数）
  - キャンセルボタン

- `RootView`: 起動時整合性チェックを追加
  - オンボーディング完了後に自動実行
  - 問題検出時にアラート表示
  - 「設定画面で復旧」ボタンでタブ切り替え
...
### 受け入れ条件達成
- ✅ ローカルデータを削除して再構築できる
- ✅ Google Calendarから全データが取得される
- ✅ インデックスが正しく再構築される
- ✅ 復旧プロセスの進捗が表示される
- ✅ 設定画面から手動実行できる
- ✅ 起動時の自動検証と破損検知時の自動復旧提案
...
### 改善内容
- **ジャーナルエディタにタグ自動補完機能を実装**
  - 本文で`#`を入力すると、過去のエントリから使用されたタグの候補を表示
  - 頻度順で上位10件のタグを候補として提示
  - 入力中の文字列で候補をフィルタリング（部分一致）
  - タグ候補をタップすると自動的に本文に挿入

- **JournalEditorViewに初期日付パラメータを追加**
  - 新規作成時に任意の日付を初期値として渡せるように
  - 将来的にFABタップ時のフォーカス日付を渡す機能の準備

### 実装詳細
- `JournalEditorView.init(entry:initialDate:)`: 初期日付パラメータを追加
- `suggestedTags`: 過去エントリからタグを収集し頻度順にソート
- `currentPartialTag`: 現在入力中の部分タグを検出
- `filteredTagSuggestions`: 入力に基づいてタグ候補をフィルタ
- `insertTag(_:)`: 選択されたタグを本文に挿入
