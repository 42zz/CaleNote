...
### 6.6 タイムラインUI表現（Googleカレンダー風）

タイムライン表示はGoogleカレンダーの「スケジュール」ビューに寄せたUI表現とする。

**日付ヘッダー**:
- 視覚的に強調し、「今日・曜日・日付」が一目で分かるデザイン
- 例: 「今日 月曜日 2024年12月23日」形式

**イベント行**:
- カード表現は弱め、影や余白を抑え、イベント行寄りの密度感
- 左の4pxカラーバーは維持、タイムライン一覧ではアイコン非表示のまま
- 時間表示は左寄せ、タイトルを主、本文スニペットは副情報として控えめに表示

**重要**: この変更はUIレイヤーのみで行い、Timeline UI Indexのフィールド構成は変更しない。
...
* **表示対象カレンダー集合**（`CachedCalendar.isEnabled == true`のカレンダーID集合）

これらはUserDefaultsまたは専用軽量テーブルで保持し、UIは状態を購読するだけにする。状態変化があっても、タイムラインの全再構築を誘発しない設計にする。

**重要**: カレンダーON/OFF切り替え時は、Timeline UI Index・Search Index・Raw Storeを再生成してはならない。State Storeに「表示対象カレンダー集合」を保持し、Timeline View Window（非永続）の表示フィルタのみを更新する。Indexは永続キャッシュであり、表示切替のたびに触らないことを仕様として保証する。
...
- Googleカレンダーの「スケジュール」ビューに寄せたUI表現
- カードの背景色はユーザーが選択したカラーパレットから適用
- カードのアイコンは、設定でカレンダーごとに選択したアイコンパレットから適用（タイムライン一覧では非表示）
- 日付区切りを表示（視覚的に強調）
- **時間表示**: 時間指定イベントは時刻を左寄せで表示、終日イベントは「終日」と表示
- **タイトル**: 主情報として強調表示
- **本文スニペット**: 副情報として控えめに表示

**UI表現の詳細**:
- 日付ヘッダーは視覚的に強調し、「今日・曜日・日付」が一目で分かるデザイン
- カード表現は弱め、影や余白を抑え、イベント行寄りの密度感
- 左の4pxカラーバーは維持、タイムライン一覧ではアイコン非表示のまま
- 時間表示は左寄せ、タイトルを主、本文スニペットは副情報として控えめに表示

**注意**: タイムライン表示の詳細は[6. タイムライン機能（軽量表示の設計）](#6-タイムライン機能軽量表示の設計)を参照。この変更はUIレイヤーのみで行い、Timeline UI Indexのフィールド構成は変更しない。

### 10.3 検索機能

- Top Barの右側に検索ボタンを配置
- 検索ボタンをタップすると検索バーがドロワー形式で表示される
...
#### 11.2.1 画面構造
メイン画面は常にスケジュールビュー（タイムライン）を表示する。下部タブバーは廃止し、**TimelineView + Sidebar + TopBar + FAB**を正式構成とする。

#### 11.2.2 構成要素

**Top Bar（画面上部）**:
- **左**: サイドバートグル（ハンバーガーメニューアイコン）
- **中央**: 現在表示している月（例: 2024年12月）
  - Timeline View Windowの先頭付近にあるdateKeyから算出
  - Raw StoreやIndexを再走査してはならない（UIのスクロール位置に基づく計算のみ許可）
- **右**: 検索ボタン、今日フォーカスボタン
  - 検索ボタン: 検索バーをドロワー形式で表示
  - 今日フォーカスボタン: タップ時に今日のdateKeyセクションへスクロール
  - 検索中は自動フォーカスを無効化（検索結果の先頭表示を維持）

**サイドバー（オーバーレイ型）**:
- 左上のトグルから開閉
- iPhone想定のオーバーレイ型（画面左側からスライドイン）
- 責務:
  - 表示するカレンダーのON/OFF切り替え（単一Googleアカウント前提、`CachedCalendar.isEnabled`を操作）
  - 設定画面への遷移
  - フィードバック導線
  - 開発者向けツール（開発者モード有効時のみサイドバー下部、または「設定内」に配置）

**タイムライン**:
- 逆時系列で表示（新しいエントリが上、古いものが下）
- Googleカレンダーの「スケジュール」ビューに寄せたUI表現
- 日付セクション + イベント行の構造（dateKeyによる日付セクション設計は維持）
- 日付ヘッダーは視覚的に強調し、「今日・曜日・日付」が一目で分かるデザイン
- カード表現は弱め、影や余白を抑え、イベント行寄りの密度感
- 左の4pxカラーバーは維持、タイムライン一覧ではアイコン非表示のまま
- 時間表示は左寄せ、タイトルを主、本文スニペットは副情報として控えめに表示
...
**FAB（新規作成ボタン）**:
- 右下に常時表示（Googleカレンダー準拠）
- サイドバー表示中は誤タップ防止のため、半透明＋非活性
...
#### 11.2.3 インタラクション
- サイドバートグルタップ → サイドバーを開閉
- 検索ボタンタップ → 検索バーがドロワー形式で表示、キーボード表示、検索実行
- 今日フォーカスボタンタップ → 今日のdateKeyセクションへスクロール
- 予定カードタップ → 予定詳細（読み取りのみ）
- ジャーナルカードタップ → ジャーナル詳細
- FABタップ → 新規作成画面（サイドバー表示中は非活性）
- 下スクロール → 過去のエントリを読み込み
- pull-to-refresh → 手動同期実行
- サイドバー内のカレンダーON/OFF切り替え → Timeline View Windowの表示フィルタのみ更新（Index再生成禁止）
...
- **シンプル**: 常時メイン表示はスケジュールビュー（タイムライン）、サイドバーで設定・カレンダー管理
- **モードン**: iOS標準に準拠しつつ洗練されたデザイン、Googleカレンダー（iOSアプリ）の操作感に寄せる
- **高速**: 入力から保存まで最短ステップ
- **文章が主役で、UIは邪魔をしないこと**
...
### 14.7 Phase 7: UI/UX再設計（Googleカレンダー風） 🔄 進行中
**目標**: ユーザー体験をGoogleカレンダー（iOSアプリ）の操作感に寄せる

- [ ] 画面構造の変更（下部タブバー廃止、サイドバー導入）
- [ ] Top Barの再構成（サイドバートグル、現在表示月、検索ボタン、今日フォーカスボタン）
- [ ] オーバーレイ型サイドバーの実装（カレンダーON/OFF切り替え、設定画面への遷移）
- [ ] タイムラインUI表現の変更（Googleカレンダー「スケジュール」ビュー風）
- [ ] FABの実装（右下常時表示、サイドバー表示中は半透明＋非活性）
- [ ] State Storeに「表示対象カレンダー集合」を追加
- [ ] カレンダーON/OFF切り替え時の表示フィルタ実装（Index再生成禁止）

**注意**: この変更はUIレイヤーのみで行い、データ設計（Raw Store / Timeline UI Index / Search Index / State Store）は原則そのまま維持する。

### 14.8 Phase 8: 将来の拡張（公開後）
- [ ] ウィジェット対応
...
**カレンダーON/OFF切り替え時の禁止事項**:
- Timeline UI Index・Search Index・Raw Storeを再生成してはならない
- State Storeに「表示対象カレンダー集合」を保持し、Timeline View Window（非永続）の表示フィルタのみを更新する
- Indexは永続キャッシュであり、表示切替のたびに触らないことを仕様として保証する
...
### 16.4 カレンダーON/OFF切り替えでの禁止事項

カレンダーON/OFF切り替え時に、Timeline UI Index・Search Index・Raw Storeを再生成してはならない。State Storeに「表示対象カレンダー集合」を保持し、Timeline View Window（非永続）の表示フィルタのみを更新する。

**禁止される実装パターン**:
```swift
// ❌ 禁止: カレンダーON/OFF切り替え時にIndexを再構築
func toggleCalendar(calendarId: String, isEnabled: Bool) {
    updateCachedCalendar(calendarId: calendarId, isEnabled: isEnabled)
    rebuildTimelineIndex() // 全件再構築は禁止
    rebuildSearchIndex() // 全件再構築は禁止
}

// ❌ 禁止: Raw Storeを再フェッチしてフィルタリング
func getVisibleEvents() -> [Event] {
    let allEvents = try modelContext.fetch(FetchDescriptor<RawCalendarEvent>())
    let enabledCalendars = getEnabledCalendars()
    return allEvents.filter { enabledCalendars.contains($0.calendarId) }
}
```

**推奨される実装パターン**:
```swift
// ✅ 推奨: State Storeに表示対象カレンダー集合を保持
func toggleCalendar(calendarId: String, isEnabled: Bool) {
    updateCachedCalendar(calendarId: calendarId, isEnabled: isEnabled)
    updateVisibleCalendarSet(calendarId: calendarId, isEnabled: isEnabled) // State Store更新のみ
    // Indexは触らない
}

// ✅ 推奨: Timeline View Windowの表示フィルタのみ更新
func getVisibleRows() -> [TimelineRowIndex] {
    let visibleCalendarIds = getVisibleCalendarSet() // State Storeから取得
    let rows = try modelContext.fetch(
        FetchDescriptor<TimelineRowIndex>(
            predicate: #Predicate { visibleCalendarIds.contains($0.calendarId) },
            sortBy: [SortDescriptor(\.sortKeyPrimary, order: .reverse),
                     SortDescriptor(\.sortKeySecondary, order: .reverse)]
        )
    )
...
| State Store | アプリの動作状態（同期状態、同期待ちキュー、競合フラグ、取得済みレンジ集合、syncToken、表示対象カレンダー集合等）を管理する軽量ストア |
